...
server <- function(input,output){
  ...
  #homePath <- paste(here(),"landSuitabilityRatingSystem",sep="/") #home path for local computer
  homePath <- here() #home path for server
  ...
  resultsPath <- paste0("results/") #where all the results files are located on the filesystem
